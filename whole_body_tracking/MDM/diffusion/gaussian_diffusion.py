...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        **kargs,
        ...
        self.rescale_timesteps = self.diff_cfg.rescale_timesteps      
    ...
    def p_mean_variance(
        self, model, x, t, clip_denoised=True, denoised_fn=None, cond_fn_x_start=None, model_kwargs=None
    ):
        """
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            if cond_fn_x_start:
                gradient = cond_fn_x_start(x, self._scale_timesteps(t), **model_kwargs)
                # 3. --- STABILIZATION: GRADIENT CLIPPING ---
                
                pred_xstart = pred_xstart + gradient
    ...
    def p_sample(
        ...
        out = self.p_mean_variance(
            ...
            cond_fn_x_start=cond_fn_x_start,
    ...
    def training_losses(self, model, 
                        x_start,
                        t, model_kwargs=None, noise=None, dataset=None):
        """
        ...
        if self.cfg.model.independant_noise:
            x_t_a =  self.q_sample(x_start[:, :, :action_dim], t[:, 0], noise=noise[:, :, :action_dim, :])            
            x_t_s =  self.q_sample(x_start[:, :, action_dim:], t[:, 1], noise=noise[:, :, action_dim:, :])
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output_action = model_output[:, :, :action_dim, :]            
            model_output_robot = model_output[:, :, action_dim:, :]


            assert model_output.shape == target.shape == x_start.shape  # [bs, njoints, nfeats, nframes]               
            assert model_output_action.shape == target_action.shape 
            assert model_output_robot.shape == target_robot.shape 

           
        
            terms["rot_mse_action"] = self.masked_l2(target_action, model_output_action, model_kwargs['y']['mask_actions']) # mean_flat(rot_mse)            
            terms["rot_mse_robot"] = self.masked_l2(target_robot, model_output_robot, model_kwargs['y']['mask']) # mean_flat(rot_mse)


            if self.lambda_target_vel > 0.:
                assert 'target_vel' in model.cond_mode
                ...
                terms["target_vel"] = masked_l2(target, pred_vel, torch.ones_like(target), entries_norm=False)
                            

            terms["loss"] = terms["rot_mse_action"] + terms["rot_mse_robot"] + terms.get('vb', 0.) \
