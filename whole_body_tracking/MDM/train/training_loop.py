...
class TrainLoop:
    def __init__(self, cfg, train_platform, model, diffusion, data):
        ...
        self.model_for_eval = self.model_avg if self.train_cfg.use_ema else self.model
        
        if self.train_cfg.gen_guidance_param != 1:
        ...
        if self.train_cfg.use_ema:
            self.opt = AdamW(
                ...
                betas=(0.9, self.train_cfg.adam_beta2),
            )
        else:
            self.opt = AdamW(
    ...
    def run_loop(self):
        ...
        for epoch in range(self.num_epochs):
            ...
            for a,c,s, cond in tqdm(self.data):
                ...
                action_batch = a.to(self.device)                
                context_batch = c.to(self.device)                
                state_batch = s.to(self.device)
    ...
    def update_average_model(self):
        ...
        if self.train_cfg.use_ema:
            ...
            for param, avg_param in zip(params, self.model_avg.parameters()):
                ...
                avg_param.data.mul_(self.train_cfg.avg_model_beta).add_(
                    param.data, alpha=1 - self.train_cfg.avg_model_beta)
    ...
    def forward_backward(self, batch, cond):
        ...
        for i in range(0, batch.shape[0], self.microbatch):
            ...
            compute_losses = functools.partial(
                ...
                self.ddp_model,
                micro,  # action/robot [bs, ch, image_size, image_size] 
    ...
    def find_resume_checkpoint(self) -> Optional[str]:
        ...
        matches = {file: re.match(r'model(\d+).pt$', file) for file in os.listdir(self.train_cfg.save_dir)}
        models = {int(match.group(1)): file for file, match in matches.items() if match}

        return pjoin(self.train_cfg.save_dir, models[max(models)]) if models else None
