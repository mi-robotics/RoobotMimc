...
from .nn import mean_flat, append_dims, append_zero, sum_flat
from .random_util import get_generator

def diff_l2(a, b):
    return (a - b) ** 2

def masked_l2(a, b, mask, weights, loss_fn=diff_l2, epsilon=1e-8, entries_norm=True):
    # assuming a.shape == b.shape == bs, J, Jdim, seqlen
    # assuming mask.shape == bs, 1, 1, seqlen
    loss = weights*loss_fn(a, b)
...
class KarrasDenoiser:
    def __init__(
        ...
        self.lambda_target_vel = self.cfg.model.lambda_target_vel
    ...
    def training_losses(self, model, x_start, sigmas, model_kwargs=None, noise=None):
        action_dim = model_kwargs['y']['action_dim']
        ...
        if self.cfg.model.independant_noise:
            x_t_a =  x_start[:, :, :action_dim] + noise[:, :, :action_dim] * append_dims(sigmas[:, 0], dims)
            x_t_s =  x_start[:, :, action_dim:] + noise[:, :, action_dim:] * append_dims(sigmas[:, 1], dims)
            x_t = torch.cat((x_t_a, x_t_s), dim=-2)
        else:
        ...
        target_action = x_start[:, :, :action_dim, :]        
        target_robot = x_start[:, :, action_dim:, :]   

        model_output_action = denoised[:, :, :action_dim, :]            
        model_output_robot = denoised[:, :, action_dim:, :]

        snrs_0 = self.get_snr(sigmas[:, 0])        
        snrs_1 = self.get_snr(sigmas[:, 1])
        weights_0 = append_dims(
            get_weightings(self.weight_schedule, snrs_0, self.sigma_data), dims
        )
        weights_1 = append_dims(
            get_weightings(self.weight_schedule, snrs_1, self.sigma_data), dims
        )

        terms["mse_action_raw"] = masked_l2(
            target_action, 
            model_output_action,
            model_kwargs['y']['mask_actions'],
            torch.ones_like(weights_0)
            ) # mean_flat(rot_mse)  
        terms["mse_action"] = masked_l2(
            target_action, 
            model_output_action,
            model_kwargs['y']['mask_actions'],
            weights_0
        ...
        terms["mse_robot_raw"] = masked_l2(
            target_robot, 
            model_output_robot, 
            model_kwargs['y']['mask'],
            torch.ones_like(weights_1)
            ) # mean_flat(rot_mse)
        
        terms["mse_robot"] = masked_l2(
            target_robot, 
            model_output_robot, 
            model_kwargs['y']['mask'],
            weights_1
        ...
        if self.lambda_target_vel > 0.:
            assert 'target_vel' in self.cfg.model.cond_mode
            target = model_kwargs['y']['target_vel']
            pred_vel = self.extract_predicted_vel(model_output_robot, self.dataset, self.cfg.model.prediction_type)
            terms["target_vel_raw"] = masked_l2(
                target, 
                pred_vel, 
                torch.ones_like(target), 
                torch.ones_like(weights_1.squeeze().unsqueeze(1) ), 
                entries_norm=False
            )
       
            terms["target_vel"] = masked_l2(
                target, 
                pred_vel, 
                torch.ones_like(target), 
                weights_1.squeeze().unsqueeze(1)  if self.cfg.model.apply_weights_to_conds else torch.ones_like(weights_1.squeeze().unsqueeze(1) ), 
                entries_norm=False
            )


        terms["loss"] = terms["mse_action"] + terms["mse_robot"] \
                             + (self.lambda_target_vel * terms.get('target_vel', 0.)) 
        


        return terms
    
    def extract_predicted_vel(self, output_robot, dataset, prediction_type):
        if prediction_type == 'state':
            output_robot_unnorm = dataset.inv_transform_s(output_robot)
        elif prediction_type == 'context':
            output_robot_unnorm = dataset.inv_transform_c(output_robot)
        else:
            raise ValueError()
        
        pred_delta = output_robot_unnorm[:, 0, dataset.velocity_indexes, :]
        pred_total_delta = pred_delta.sum(dim=-1)
        pred_vels = pred_total_delta/output_robot_unnorm.size(-1)*50
    ...
    def denoise(self, model, x_t, sigmas, **model_kwargs):
        ...
        if not self.distillation:
            if self.cfg.model.independant_noise:
                return self.denoise_independant(model, x_t, sigmas, **model_kwargs)
            c_skip, c_out, c_in = [
    ...
    def denoise_independant(self, model, x_t, sigmas, **model_kwargs):
        action_dim = model_kwargs['y']['action_dim']

        c_skip_0, c_out_0, c_in_0 = [
            append_dims(x, x_t.ndim)
            for x in self.get_scalings(sigmas[:, 0])
        ]

        c_skip_1, c_out_1, c_in_1 = [
            append_dims(x, x_t.ndim)
            for x in self.get_scalings(sigmas[:, 1])
        ]

        rescaled_t_0 = 1000 * 0.25 * th.log(sigmas[:, [0]] + 1e-44)        
        rescaled_t_1 = 1000 * 0.25 * th.log(sigmas[:, [1]] + 1e-44)
        rescaled_t = torch.cat((rescaled_t_0, rescaled_t_1), dim=-1)

        x_t_a = x_t[:, :, :action_dim, :] 
        x_t_a_in = x_t_a * c_in_0        
        x_t_s = x_t[:, :, action_dim:, :]
        x_t_s_in = x_t_s * c_in_1

        x_t_in_mixed = torch.cat((x_t_a_in, x_t_s_in), dim=-2)
        model_output = model(x_t_in_mixed, rescaled_t, **model_kwargs)

        x_t_a_skip = x_t_a * c_skip_0        
        x_t_s_skip = x_t_s * c_skip_1
        x_t_skip_mixed = torch.cat((x_t_a_skip, x_t_s_skip), dim=-2)

        mo_a = model_output[:, :, :action_dim, :] * c_out_0        
        mo_s = model_output[:, :, action_dim:, :] * c_out_1
        model_output_mixed = torch.cat((mo_a, mo_s), dim=-2)

        denoised = model_output_mixed + x_t_skip_mixed
        return model_output, denoised
